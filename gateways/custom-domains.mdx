...
<Steps>
  <Step title="Choose your domain">
    Decide which domain or subdomain you want to use for your gateway. Common choices include:
    - `cdn.yourdomain.com`
  ...
  <Step title="Add a CNAME record">
    In your DNS provider's management console, create a `CNAME` record pointing your chosen domain to your Tusky gateway:

    | Type | Name | Value | TTL |
    | --- | --- | --- | --- |
    | `CNAME` | `cdn` | `{subdomain}.mytusky.xyz` | 3600 |

    Replace `{subdomain}` with your Tusky gateway subdomain.
  ...
  <Step title="Configure in Tusky">
    ...
    <Tabs>
      <Tab title="Dashboard">
        Navigate to **Settings → Gateway → Custom Domain** and enter your domain (e.g. `cdn.yourdomain.com`).
      </Tab>
      <Tab title="SDK">
        ```typescript
        await tusky.gateway.update({
...
The response should show your Tusky gateway as the CNAME target.
...
<Tabs>
  <Tab title="Dashboard">
    Navigate to **Settings → Gateway → Custom Domain** and click **Remove**.
  </Tab>
  <Tab title="SDK">
    ```typescript
    await tusky.gateway.update({
...
<CardGroup cols={2}>
  <Card title="Gateway Overview" icon="network-wired" href="/gateways/overview">
    Understand how the full gateway infrastructure works.
  </Card>
