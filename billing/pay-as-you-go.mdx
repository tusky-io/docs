...
<Steps>
  <Step title="Estimate cost">
    Before uploading, use the [upload cost endpoint](/api-reference/uploads/upload-cost) to preview the cost:
...
const file = await tusky.file.upload("vol_abc123", "./data.csv", {
  name: "dataset.csv",
  epochs: 5,
});
