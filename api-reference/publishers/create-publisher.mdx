...
Create a new publisher. Each account gets one active publisher automatically on sign-up. You can create additional publishers with different configurations and attach them to specific drives.
...
<ParamField body="driveId" type="string">
  Optional drive ID to attach this publisher to. When set, this publisher configuration is used for all uploads to the specified drive, overriding the account default.
</ParamField>
...
<Note>
  When a drive has an attached publisher, uploads to that drive use the attached publisher's configuration instead of the account default.
</Note>
