...
Create a new publisher. Each account gets one active publisher automatically on sign-up. You can create additional publishers with different configurations and attach them to specific volumes.

## Request Body

<ParamField body="state" type="string" default="ready">
...
<ParamField body="autoConvert" type="boolean" default="true">
...
<ParamField body="volumeId" type="string">
  Optional volume ID to attach this publisher to. When set, this publisher configuration is used for all uploads to the specified volume, overriding the account default.
</ParamField>
...
<Note>
  When a volume has an attached publisher, uploads to that volume use the attached publisher's configuration instead of the account default.
</Note>
