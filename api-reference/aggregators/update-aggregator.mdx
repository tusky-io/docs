...
<ParamField body="fallbackToBackup" type="boolean">
  Whether to fall back to the centralized backup store when Walrus is unavailable. Requires [Backup](/tools/backup) to be enabled on the relevant drives.
</ParamField>
...
<Note>
  `fallbackToBackup` only works for drives with [Backup](/tools/backup) enabled. If backup is not enabled, the flag is accepted but has no effect.
</Note>
