...
<ParamField body="restricted" type="boolean">
  When `true`, requests must be authenticated. When `false`, content is served publicly.
</ParamField>

<ParamField body="restrictedMode" type="string">
  Controls how clients pass the access token when `restricted` is `true`. One of: `header` (default — `Api-Key` request header), `query` (`?api-key=` query parameter), or `path` (`/{accessToken}/v1/blobs/{blobId}` URL prefix).
</ParamField>
...
<ParamField body="fallbackToBackup" type="boolean">
  Whether to fall back to the centralized backup store when Walrus is unavailable. Requires [Backup](/tools/backup) to be enabled on the relevant volumes.
</ParamField>
...
<Note>
  `fallbackToBackup` only works for volumes with [Backup](/tools/backup) enabled. If backup is not enabled, the flag is accepted but has no effect.
</Note>
