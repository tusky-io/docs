...
<ParamField query="vaultId" type="string" required>
  Target drive ID. Can also be provided in `Upload-Metadata`.  Query params take precedence.
</ParamField>

<ParamField query="parentId" type="string">
  Parent folder ID. Defaults to drive root. Can also be in `Upload-Metadata`.
</ParamField>
...
<Tabs>
  <Tab title="Single request (multipart)">
    ```bash
    curl -X POST "https://api.tusky.io/v2/uploads?vaultId=drive_abc123&filename=photo.jpg" \
  ...
  <Tab title="Single request (binary)">
    ```bash
    curl -X POST "https://api.tusky.io/v2/uploads?vaultId=drive_abc123&filename=photo.jpg&filetype=image/jpeg" \
      -H "Authorization: Bearer YOUR_API_KEY" \
  ...
  <Tab title="Resumable (create only)">
    ...
    curl -X POST https://api.tusky.io/v2/uploads \
      ...
      -H "Upload-Metadata: filename $(echo -n 'video.mp4' | base64),vaultId $(echo -n 'drive_abc123' | base64),filetype $(echo -n 'video/mp4' | base64)" \
      -D -
    ```
