...
<Steps>
  ...
  <Step title="Start migration">
    Tusky begins pulling objects from S3 and uploading them to your chosen drive on the Walrus network. Progress is tracked in real time.
  </Step>
...
<Steps>
  ...
  <Step title="Select target drive">
    Choose an existing drive or create a new one to receive the migrated files.
  </Step>
...
<Note>
  Tusky preserves the original file names, directory structure (prefixes), and MIME types from S3. Your files are organized in the target drive exactly as they were in the bucket.
</Note>
