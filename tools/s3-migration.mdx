...
<Steps>
  ...
  <Step title="Start migration">
    Tusky begins pulling objects from S3 and uploading them to your chosen vault on the Walrus network. Progress is tracked in real time.
  </Step>
  <Step title="Verify and switch">
    Once the migration completes, verify your content through your Tusky gateway and update your application to point to the new URLs.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Select target vault">
    Choose an existing vault or create a new one to receive the migrated files.
  </Step>
...
1. **Verify content** — Spot-check files through your Tusky gateway to confirm integrity.
2. **Update references** — Replace S3 URLs in your application with Tusky gateway URLs.
...
<Note>
  Tusky preserves the original file names, directory structure (prefixes), and MIME types from S3. Your files are organized in the target vault exactly as they were in the bucket.
</Note>
