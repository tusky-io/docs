...
Tusky supports two authentication methods, each designed for different use cases. You can authenticate with an **API key** for server-side applications and scripts, or with a **Sui wallet** for browser-based dApps.
...
- Store keys in environment variables (e.g. `TUSKY_API_KEY`).
