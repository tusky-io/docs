...
<Tip>
  Use the [cost estimation endpoint](/api-reference/uploads/create-upload) to preview the storage cost before uploading. Cost = Walrus storage cost (WAL) + gas fee (SUI). Tusky charges no infrastructure fee.
</Tip>
...
<Note>
  For encrypted environments, the SDK automatically decrypts the file content using keys derived from your wallet or API key. No additional steps are required.
</Note>
...
<CardGroup cols={2}>
  <Card title="Environments" icon="vault" href="/sdk/environments">
    Manage the environments that contain your files.
  </Card>
