...
API keys let you interact with Tusky programmatically through the [REST API](/api-reference/introduction) and the [TypeScript SDK](/sdk/getting-started). Each key is scoped to your account and can be configured with specific permissions.
...
| `gateways:read` | Read gateway configuration |
| `gateways:write` | Update gateway settings and custom domains |

<Tip>
  Follow the **principle of least privilege**: only grant the scopes your application needs. A key used solely for reading files should not have `vaults:write` or `members:write` permissions.
</Tip>
...
```bash
curl https://api.tusky.io/v1/vaults \
