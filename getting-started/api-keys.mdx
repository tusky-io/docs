...
<Tip>
  Follow the **principle of least privilege**: only grant the scopes your application needs. A key used solely for reading files should not have `drives:write` or `members:write` permissions.
</Tip>
...
```bash
curl https://api.tusky.io/v1/drives \
