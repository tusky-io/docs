...
<CardGroup cols={2}>
  ...
  <Card title="End-to-End Encryption" icon="lock">
    Every file is encrypted before it leaves your device. Encryption keys are tied to your connected Sui wallet or managed through [Seal](https://docs.sui.io/standards/seal), eliminating password friction entirely.
  </Card>
  <Card title="Vaults & File Management" icon="folder-tree">
    Organize files into vaults — encrypted containers with granular access control. Vaults give you a familiar file-system abstraction on top of Walrus blobs.
  </Card>
  <Card title="Private Gateways" icon="shield-halved">
    Each user gets a private gateway with custom subdomains or fully custom domains. Serve content directly from the Walrus network through your own branded endpoint.
  </Card>
  <Card title="SDK & API" icon="code">
    A TypeScript SDK and REST API let you integrate Tusky into any application. Upload, download, manage vaults, and control access programmatically.
  </Card>
...
You interact with Tusky through the [dashboard](https://app.tusky.io), the [TypeScript SDK](/sdk/getting-started), or the [REST API](/api-reference/introduction). Under the hood, Tusky handles blob encoding, storage node coordination, encryption key management, and data retrieval.
...
| Concept | Description |
| --- | --- |
| **Vaults** | Encrypted containers that group files with shared access policies. |
| **Files** | Individual objects stored inside vaults, encrypted and published to Walrus. |
| **Members** | Users or API keys granted access to a vault with role-based permissions. |
| **Gateways** | Private HTTP endpoints for serving vault content over custom domains. |
