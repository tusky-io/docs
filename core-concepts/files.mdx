...
<Steps>
  <Step title="Select a drive">
    Choose the drive where the file will live. The drive's encryption setting determines whether the file is encrypted before upload.
  </Step>
  <Step title="Upload the file">
    Drag-and-drop or select a file from your device. If the drive is encrypted, Tusky encrypts the file **client-side** in your browser before any data leaves your machine.
  </Step>
...
| **Drive** | The parent drive containing this file. |
...
<Tabs>
  ...
  <Tab title="Auto-extend">
    Enable **auto-extend** on a file or at the drive level. Tusky automatically renews the storage allocation before the current epochs expire, ensuring uninterrupted availability. Auto-extend charges are billed to your account.
  </Tab>
</Tabs>

<Note>
  You can change the auto-extend setting for individual files or apply it as a drive-wide default. Drive-level settings are inherited by new uploads but can be overridden per file.
</Note>
...
```
Upload → Encrypt (if private drive) → Batch into quilt → Publish to Walrus → Retrieve via aggregator
```

1. **Upload** — File data is sent to Tusky (encrypted client-side for private drives).
...
<Warning>
  If a file was uploaded to a public drive, anyone who recorded the blob ID can still retrieve it directly from Walrus until the epochs expire, even after you delete it in Tusky.
</Warning>
