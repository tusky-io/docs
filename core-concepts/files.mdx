...
Files are the fundamental units of storage in Tusky. Every file you upload is published to the [Walrus](https://walrus.xyz) decentralized storage network and served back to you through your dedicated aggregator. This page covers the upload process, supported formats, storage duration, and retrieval.

## Upload process

<Steps>
  <Step title="Select a volume">
    Choose the volume where the file will live. The volume's encryption setting determines whether the file is encrypted before upload.
  </Step>
  <Step title="Upload the file">
    Drag-and-drop or select a file from your device. If the volume is encrypted, Tusky encrypts the file **client-side** in your browser before any data leaves your machine.
  </Step>
...
| **Volume** | The parent volume containing this file. |
...
<Tabs>
  ...
  <Tab title="Auto-extend">
    Enable **auto-extend** on a file or at the volume level. Tusky automatically renews the storage allocation before the current epochs expire, ensuring uninterrupted availability. Auto-extend charges are billed to your account.
  </Tab>
</Tabs>

<Note>
  You can change the auto-extend setting for individual files or apply it as a volume-wide default. Volume-level settings are inherited by new uploads but can be overridden per file.
</Note>
...
You can retrieve files using either identifier through your aggregator:
...
```
Upload → Encrypt (if private volume) → Batch into quilt → Publish to Walrus → Retrieve via aggregator
```

1. **Upload** — File data is sent to Tusky (encrypted client-side for private volumes).
...
<Warning>
  If a file was uploaded to a public volume, anyone who recorded the blob ID can still retrieve it directly from Walrus until the epochs expire, even after you delete it in Tusky.
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Aggregators" icon="network-wired" href="/core-concepts/aggregators">
    See how to access your files through your private aggregator.
  </Card>
</CardGroup>
