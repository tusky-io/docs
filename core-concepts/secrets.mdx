...
Secrets are **global to your account**, not scoped to any environment. Organize them using path hierarchy to group by service, stage, or purpose. Unlike centralized secret managers, secrets are encrypted client-side and access is controlled by on-chain smart contracts — Tusky never sees your plaintext values.
