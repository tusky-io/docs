...
Secrets can be associated with a specific environment. When a secret is scoped to an environment, it inherits the environment's access control policies — any member with access to the environment can read the secret.
...
Account-level secrets (no `environmentId`) are accessible only by the account owner.
