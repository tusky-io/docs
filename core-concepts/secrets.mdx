...
Tusky includes a built-in **secret manager** for storing sensitive key-value data — API keys, database credentials, certificates, and configuration — encrypted on-chain using [Sui Seal](https://seal-docs.wal.app).

Secrets are **global to your account**, not scoped to any volume. Organize them using path hierarchy to group by service, stage, or purpose. Unlike centralized secret managers, secrets are encrypted client-side and access is controlled by on-chain smart contracts — Tusky never sees your plaintext values.
...
Paths use `/` separators for hierarchy: `{volume}/{service}/{key}`.
...
When you create an **encrypted volume**, Tusky automatically creates a managed secret at:

```
tusky/volumes/{volumeId}/encryption
```

This secret contains the Seal encryption keys used to encrypt and decrypt files in that private volume. It is:
- **Created automatically** when an encrypted volume is created
- **Non-deletable** — attempting to delete returns `403 Forbidden`
- **Readable** — you can read the key material via the Secrets API
- **Removed only** when the parent volume is deleted
...
Secrets are encrypted using [Seal](https://seal-docs.wal.app), a decentralized encryption protocol on Sui:
