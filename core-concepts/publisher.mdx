...
A **publisher** defines how files are written to the Walrus network — storage duration, batching strategy, deletability, and sharing. Every Tusky account starts with one active publisher automatically. You can create additional publishers with different configurations and assign them to volumes.

Assigning **multiple publishers** to a single volume enables **horizontal scaling** — incoming uploads are load-balanced across publishers, each with its own managed wallet and transaction capacity. This reduces publishing latency for high-throughput workloads.
...
<Steps>
  <Step title="Account default">
    When you sign up, Tusky provisions a default publisher. All uploads use this publisher unless a volume has a specific publisher attached.
  </Step>
  ...
  <Step title="Attach to volumes">
    Publishers can be attached to specific volumes. When a volume has an attached publisher, all uploads to that volume use that publisher's configuration instead of the account default.
  </Step>
  <Step title="Upload">
    When you upload a file, Tusky uses the publisher attached to the target volume, or the account default if none is attached. The publisher's settings (epochs, quilt, permanent, share) determine how the blob is written to Walrus.
  </Step>
</Steps>
...
## Attaching publishers to volumes

You can attach a publisher to a volume to give that volume a specific publishing configuration. This is useful when different volumes have different storage requirements.
...
When a volume has an attached publisher, all uploads to that volume use the attached publisher's config. If a volume does not have a publisher attached, it uses the account's default publisher.
...
<Tabs>
  <Tab title="List publishers">
    ...
    publishers.forEach(p => {
      console.log(`${p.id}: state=${p.state}, epochs=${p.epochs}, quilt=${p.quilt}, autoConvert=${p.autoConvert}, volume=${p.volumeId || 'default'}`);
    });
    ```
  </Tab>
...
<AccordionGroup>
  ...
  <Accordion title="I want to simplify token management">
  ...
  <Accordion title="Different volumes need different settings">
    Create multiple publishers with different configs and attach each to the appropriate volume.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Files" icon="file" href="/core-concepts/files">
    Learn how files are uploaded and managed inside volumes.
  </Card>
</CardGroup>
