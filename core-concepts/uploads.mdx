...
<Steps>
  <Step title="Create the upload">
    Send a `POST` request to the upload endpoint with file metadata (size, filename, drive ID). Tusky returns a unique upload URL in the `Location` header.
  </Step>
...
| `Upload-Metadata` | Request (POST) | Base64-encoded key-value pairs: `filename`, `filetype`, `driveId`. |
...
Client                                    Server
  ...
  │  Upload-Metadata: filename ..., driveId  │
  │ ────────────────────────────────────────▶ │
  │                                          │
  │  201 Created                             │
...
const fileId = await tusky.file.upload("drive_abc123", "./large-video.mp4", {
...
<CardGroup cols={2}>
  ...
  <Card title="Encryption" icon="lock" href="/core-concepts/encryption">
    How files are encrypted before upload in private drives.
  </Card>
</CardGroup>
