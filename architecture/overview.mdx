...
Each Tusky customer gets a **dedicated aggregator** — a real Kubernetes Deployment provisioned on demand when an aggregator is created. The aggregator pod is configured with the customer's account ID so it only serves that account's content.

- **Per-customer Kubernetes Deployments** with isolated pods and ClusterIP Services
- **Per-customer DNS records** (`{subdomain}.mytusky.xyz`) provisioned via Cloudflare API
- **Fallback aggregator** on separate infrastructure for resilience
- Optimized for low-latency blob reconstruction through the Cloudflare edge

For full details, see [Aggregator Provisioning](/architecture/aggregator-provisioning).
...
<CardGroup cols={2}>
  ...
  <Card title="Aggregator Provisioning" icon="server" href="/architecture/aggregator-provisioning">
    Deep dive into per-customer Kubernetes deployments and DNS provisioning.
  </Card>
</CardGroup>
