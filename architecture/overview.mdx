...
<Steps>
  <Step title="Upload received">
    The file is sent to the Tusky REST API. For encrypted vaults, the SDK encrypts the file client-side before transmission.
  </Step>
  ...
  <Step title="Metadata recorded">
    Once published, the blob ID, quilt patch ID, and storage details are recorded in PostgreSQL. The file is now accessible through your gateway.
  </Step>
</Steps>
...
When a client requests a file through your gateway:

<Steps>
  <Step title="Request arrives">
    The client sends an HTTP request to your private gateway URL (e.g. `https://myproject.mytusky.xyz/qp_abc123`).
  </Step>
...
The central interface for all Tusky operations. Handles authentication, authorization, vault/file CRUD, and orchestrates the write and read paths.
...
Aggregators reconstruct blobs from the Walrus network and serve them through gateways:
...
| System | Purpose |
| --- | --- |
| **PostgreSQL** | Primary state store — vaults, files, members, accounts, transaction history. |
| **Redis** | Caching layer for hot metadata, rate limiting counters, and queue management. |

### CDN (Cloudflare)

All gateway traffic passes through Cloudflare:
...
<CardGroup cols={2}>
  ...
  <Card title="Gateways" icon="globe" href="/gateways/overview">
    Learn how content is delivered through the CDN and gateway infrastructure.
  </Card>
</CardGroup>
