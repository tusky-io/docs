...
```
File → Encrypt (if private vault) → Encode → Blob → Walrus Network
```
...
A **quilt patch ID** identifies a specific file within a quilt. When you access a file through your Tusky gateway using `/{quiltPatchId}`, the system locates the parent quilt blob, extracts the relevant byte range, and returns the file.
...
<Tabs>
  <Tab title="Fixed Duration">
    ...
    ```typescript
    await tusky.file.upload(vaultId, "./data.csv", {
      epochs: 10,
    });
    ```
  </Tab>
...
1. Encrypt the file (if the vault is encrypted)
...
When a file is requested through your gateway, Tusky's aggregators:
